...
public class FragmentDataGrid extends Fragment implements OnLongClickListener, OnClickListener {
    ...
    public boolean getSpecialData(int i, float[] fArr) {
        ...
        if (i == Constants.DATA_WIFI_STRENGTH) {
            ...
            fArr[0] = (float) MoveManager.getInstance().getMove().norm();
            return true;
        } else if (i == Constants.DATA_MOVE_PITCH) {
            fArr[0] = (float) (MoveManager.getInstance().getMovePitch() * Constants.RAD2DEG);
            return true;
        } else {
            return false;
        }

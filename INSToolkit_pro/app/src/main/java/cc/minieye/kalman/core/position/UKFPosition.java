...
import cc.minieye.kalman.core.attitude.UKFAttitude;
...
public final class UKFPosition extends PositionFilter {
    ...
    public UKFPosition(double d, double d2, MeasuresManager measuresManager, int i) {
        ...
        this.mUpdateTimeTask = new Runnable() {
            public void run() {
                ...
                UKFPosition.this.ukfStep(((double) (currentTimeMillis - UKFPosition.this.last_time)) / 1000.0d);
                UKFPosition.this.last_time = currentTimeMillis;
                if (UKFPosition.this.status == UKFPosition.STARTED) {
                    UKFPosition.this.mHandler.postDelayed(UKFPosition.this.mUpdateTimeTask, UKFPosition.this.period);
                }
            }
        };
        this.measuresManager = measuresManager;
        this.measuresManager.setGPSOrigin(this.OriginPosition);
        this.kattitude = new UKFAttitude(measuresManager, i);
    ...
    private void ukfStep(double d) {
        ...
        if (this.isKFPositionEnable) {
            ...
            if (this.status == STARTED) {
                this.physics.setAttitude(this.kattitude.getQuaternion(), this.kattitude.matLocalFrame2DevFrame());
    ...
    public void setStateCovariances(double d, double d2, double d3, double d4, double d5, double d6) {
        this.kattitude.setProcessCovariances(d, d2, d6);
